...
from uavdiff.obs import ObservationBuilder
...
class CollectDataEnv:
    ...
    def get_airship_center(self) -> np.ndarray:
        assert self.scene is not None, "Call reset() before get_airship_center()."
        return self.scene.airship.pos.astype(np.float32).copy()


    def force_return_uavs_to_center(
        self,
        append_traj: bool = True,
    ) -> np.ndarray:
        """
        Teleport all UAVs to the airship center without consuming an env step,
        without changing time, packets, or reward.
        Returns the new observation after teleport.
        """
        assert self.scene is not None, "Call reset() before force_return_uavs_to_center()."

        center = self.scene.airship.pos.astype(np.float32).copy()
        ...
        for i, uav in enumerate(self.scene.uavs):
            ...
            if append_traj:
                self.traj[i].append(center.copy())
