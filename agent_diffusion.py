...
from config import ExpCfg, CFG, derive_seed
from env import CollectDataEnv
from condition import ConditionBuilder
from replay_diffusion import ReplayBufferDiffusion, StepBatch, ChunkBatch
from critic_nets import CriticFactory, flatten_state_history, extract_first_action, soft_update
from diffusion_core import env_to_unit, ActionRange, make_training_target
from diffusion_nets import DiffusionPolicy
from runner_diffusion import DiffusionRunner
...
class AgentDiffusion:
    ...
    def warmup(self, warmup_steps: Optional[int] = None) -> None:
        ...
        for _ in range(n):
            ...
            next_obs, rew, done, info = self.env.step(self._flat_to_env_action(act_flat))
            ...
            self.runner.obs_history.append(next_obs)
            obs = next_obs
    ...
    def update_actor(self, batch: ChunkBatch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.cfg.diffusion.predict_type == "eps":
            from diffusion_core import predict_x0_from_eps, unit_to_env
            ...
            from diffusion_core import unit_to_env
            x0_pred_unit = pred
        else:
